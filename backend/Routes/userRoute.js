...
router.post("/user/register", [
    ...
    check('email').isEmail(),
    check('password').matches(/^(?=.*\d)(?=.*[a-z])(?=.*[A-Z])(?=.*[a-zA-Z]).{8,}$/).withMessage('Must have symbol lowercase uppercase and number').isLength({ min: 8 }).withMessage('Must be at least 8 chars long'),
